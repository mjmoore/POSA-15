...
public class ImageOpsBoundService extends ImageOpsImpl {
    ...
    public ImageOpsBoundService(MainActivity activity) {
        ...
        mReplyMessenger = new Messenger(mServiceResultHandler);
    }        
    ...
    public void bindService() {
        if (mRequestMessengerRef == null) {
            ...
            // user.  
            // DONE  - you fill in here.
            final Intent intent = DownloadImagesBoundService.makeIntent(mActivity.get());
    ...
    protected void startDownload(Uri url) {
        if (mRequestMessengerRef == null) {
            Utils.showToast(mActivity.get(), "not bound to the service");
            return;
        }

        Log.d(TAG, "sending a request message to DownloadImagesBoundService for " + url.toString());
        try {
            ...
            final RequestMessage requestMessage = RequestMessage.makeRequestMessage(
                OperationType.DOWNLOAD_IMAGE.ordinal(), url, mDirectoryPathname, mReplyMessenger);
